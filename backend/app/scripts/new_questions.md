...
`Generic[T]` делает класс параметризуемым по типу — как `list[int]` или `list[str]` являются разными вариантами одного и того же `list`, так и ваш собственный класс может быть параметризован. Без этого, если написать класс-контейнер без указания типа, тайпчекер будет считать все его элементы `Any` и не сможет поймать ошибку, если вы положите в `Stack[int]` строку или попытаетесь вызвать несуществующий у элемента метод.
...
# новый синтаксис (Python 3.12+), делает то же самое короче
class Stack[T]:
    def __init__(self) -> None:
        self.items: list[T] = []
...
int_stack.push(1)
int_stack.push("oops")  # ошибка mypy: str не подходит под Stack[int]

result: int = int_stack.pop()  # тайпчекер знает, что pop() вернёт int, а не Any
```
...
`Callable[[arg_types], return_type]` описывает тип функции или любого вызываемого объекта — то есть говорит тайпчекеру не просто "это функция", а какие у неё должны быть аргументы и что она возвращает. Нужен там, где функция или метод принимает другую функцию как аргумент (колбэки, стратегии, декораторы) — без Callable тайпчекер не сможет проверить, что вы передали функцию с правильной сигнатурой, и просто пропустит любую ошибку.
...
# ошибка тайпчекера: сигнатура не совпадает
def wrong(a: int) -> int:  # принимает 1 аргумент, а не 2
    return a

apply(wrong, 1, 2)  # mypy: Argument 1 has incompatible type
```
...
Важно: это чисто статическая проверка — на неё смотрит только тайпчекер (mypy/pyright) до запуска кода. Сам интерпретатор Python эти правила не знает и не проверяет: код ниже выполнится без единой ошибки, MAX_SIZE реально станет 200, а `Derived.important` переопределится. Ошибку покажет только команда `mypy .`, запущенная отдельно.

```python
from typing import Final, final

MAX_SIZE: Final = 100
MAX_SIZE = 200  # ошибка mypy, но код при этом выполнится без исключений

@final
class Singleton:
    pass

class Child(Singleton):  # ошибка mypy: наследование от final класса
    pass
...
class Derived(Base):
    def important(self) -> None:  # ошибка mypy: переопределение final метода
        pass
```
...
Результаты возвращаются в том же порядке что и входные аргументы — независимо от порядка завершения. 
Параметр `return_exceptions=True` позволяет не прерываться при исключении в одной из задач — вместо этого исключение попадает в результат как обычное значение. Начиная с Python 3.11 для большинства случаев предпочтительнее `TaskGroup` — он безопаснее.
...
Алгоритмы делятся на симметричные и асимметричные. Симметричные (HS256, HS384, HS512) используют один общий секретный ключ для создания и проверки подписи — подходят, когда подписывает и проверяет одна и та же система. Асимметричные (RS256, ES256, EdDSA) используют пару ключей: приватный для подписи, публичный для проверки — это позволяет нескольким сервисам проверять токены, не зная приватного ключа.

По скорости и безопасности эти два свойства нельзя смешивать в одну шкалу:

Скорость: HS256 (HMAC) — самый быстрый вариант при подписи и проверке, это один проход хеш-функции. 
У RS256, наоборот, подпись медленная (математика RSA), а проверка быстрая. 
ES256/EdDSA обычно быстрее RS256 и по подписи, и по проверке, при этом дают меньший размер подписи.
Модель доверия/безопасность на практике: HS256 требует хранить один секрет на всех, 
кто подписывает и проверяет — если утечёт, скомпрометированы обе стороны, и нет возможности дать кому-то только право проверки. 
Асимметричные алгоритмы (RS256, ES256, EdDSA) решают эту проблему: публичный ключ можно раздавать всем, приватный остаётся у эмитента токена. EdDSA и ES256 сейчас считаются современным стандартом, RS256 — универсальный, но более старый вариант с крупной подписью.

```python
# HS256 — один ключ, быстрее всех, но нет разделения "кто подписывает / кто проверяет"
token = jwt.encode(payload, key="shared_secret", algorithm="HS256")
jwt.decode(token, key="shared_secret", algorithms=["HS256"])

# RS256 — пара ключей, подпись медленнее, зато проверяющих может быть много
