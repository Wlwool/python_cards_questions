...
`==` сравнивает значения объектов (вызывает `__eq__`), `is` сравнивает идентичность — один ли это объект в памяти (эквивалентно id(a) == id(b)).
...
c = a
c is a   # True — это один и тот же объект
```

Подвох с кешированием: CPython кеширует маленькие целые числа (обычно от -5 до 256) и некоторые строки-идентификаторы (интернирование), поэтому is может неожиданно вернуть True там, где вы этого не ожидали — но полагаться на это нельзя, это деталь реализации CPython, а не гарантия языка:

```python
x = 256
y = 256
x is y   # True — маленькие int закешированы

x = 257
y = 257
x is y   # False (в интерпретаторе, не в одной строке кода) — уже не закешировано
```

Практическое правило: is используют только для сравнения с None, True, False (if x is None, а не if x == None) — потому что это синглтоны, и так быстрее и однозначнее. Для всего остального — ==.


### Какие виды строк бывают в питоне

Во второй версии питона было два типа: однобайтные строки и Юникод представлены классами str и Unicode соответственно. В третьем Питоне есть один вид строк str, который представляет собой Юникод. Однобайтных строк нет, вместо них есть тип bytes, то есть цепочка байт.
...
Декодировать – восстановить строку из цепочки байт. Вызвать метод `.decode()` у объекта `str` или `bytes`.
...
Диапазоны – неизменяемые последовательности чисел, которые задаются началом, концом и шагом. Представлены классом range.
...
В Python эти методы создают объект-представление. Но итерирование по такому представлению должно происходить немного дольше, чем по списку из-за того что данные в словарях хранятся разреженно (редко, негусто). В подтверждение вышесказанного (Python 3):
...
Ключ. Порядок следования ключей не гарантируется (в 3.6 гарантируется неофициально, в 3.7 гарантируется). Для маленьких словарей порядок будет тот же, что и в объявлении. Для больших словарей порядок зависит от расположения элементов в памяти. Особый класс `OrderedDict` учитывает порядок добавления ключей.
...
Функция super() даёт доступ к методам родительского класса. В Python 3 есть два варианта - короткий (рекомендуемый) и явный:

```python
class NextClass(FirstClass):
    def __init__(self, x):
        super().__init__()          # современный способ, Python 3+
        # super(NextClass, self).__init__()  # старый явный способ, тот же результат
        self.x = x
```
Явная форма super(NextClass, self) нужна редко — например, когда важно вызвать метод конкретного класса в цепочке множественного наследования, а не следующий по MRO. 
В остальных случаях используют короткую форму super().
...
*MRO* – method resolution order, порядок разрешения методов. Алгоритм, по которому следует искать метод в случае, если у класса два и более родителя.
...
Миксин (mix-in с англ. “Примесь”), паттерн проектирования в ООП, когда в цепочку наследования добавляется небольшой класс-помощник. Например, есть класс
...
Недостаток: нельзя присвоить классу поле, которого нет в слотах. Не работают методы `__getattr__` и `__setattr__`.
...
- Причиной создания new style classes послужила идея убрать отличие встроенных типов от определённых пользователем типов.
...
Код на Python может сгенерировать исключение при помощи ключевого слова raise. После него указывается объект исключения. Также можно указать класс исключения, в таком случае будет автоматически вызван конструктор без параметров. А raise может выбрасывать в качестве исключений только экземпляры класса BaseException и его наследников.
...
- Базовые:
  ...
  - Exception – класс-наследник BaseException, базовый класс для всех стандартных исключений, которые не указывают на обязательное завершение программы, и всех пользовательских исключений.
  ...
  - OSError – базовый класс для исключений, связанных с ошибками, которые происходят вне интерпретатора Python.
...
В Python всё является объектом, в том числе сами классы.
Когда ты пишешь `class MyClass: ...`, Python вызывает метакласс,
чтобы создать объект-класс. По умолчанию это `type`.
Используется редко — для фреймворков (Django ORM, SQLAlchemy), автоматической регистрации классов, валидации при определении класса.
В большинстве случаев вместо метакласса достаточно __init_subclass__ или декоратора класса.
```python
# Обычный способ
class Foo:
    pass

# То же самое, но явно через type(name, bases, attrs)
Foo = type('Foo', (), {})

# Свой метакласс — переопределяем создание класса
class MyMeta(type):
...
Однако если вы выполните следующий код вы получите не `IntegerField`, а `int`, причём значение может быть получено прямо из базы данных.
...
В Python существует три вида файловых объектов: текстовые файлы (text files), «обычные» (небуферизированные) бинарные файлы (raw binary files) и буферизированные бинарные файлы (buffered binary files). Разные виды потоков представляются соответствующими классами модуля io.
...
Режим mode может начинаться с символов «r» (чтение), «w» (запись, очищает файл, если уже существует), «x» (исключительное создание, неуспешно, если файл уже существует), «a» (добавление, запись в конец файла).
...
Да, являются: у них реализованы __enter__ и __exit__, поэтому их можно использовать через with. 
__exit__ гарантированно закрывает файл при выходе из блока — даже если внутри произошло исключение. 
Это надёжнее, чем вручную звать close(), потому что при ошибке до close() можно просто не дойти.

```python
with open('file.txt') as f:
    data = f.read()
# файл уже закрыт здесь, даже если read() выбросил исключение

# без контекстного менеджера пришлось бы писать так:
f = open('file.txt')
try:
    data = f.read()
finally:
...
- по возможности подготовил бы для теста такой входящий набор данных, при котором она выполнялась быстро

Если функция не соответствует условиям, описанным в первом предложении, следовало бы сначала заняться ее декомпозицией.
...
#### Проверка работоспособности (Smoke test, Sanity check)
  ...
  Это частный случай интеграционного теста. Обычно это очень небольшие тесты, которые прогоняются перед запуском системы, чтобы убедиться в работоспособности стороннего ПО, это необходимо для корректного функционирования нашей системы. В случае провала таких тестов, мы можем оповестить пользователя о проблеме или и вовсе остановить запуск системы.
...
### Что такое fixture, parametrize и conftest.py в pytest

`Fixture` — функция, которая готовит данные или ресурс для теста (подключение к БД, тестовый объект, временный файл) и убирает за собой после. Декоратор @pytest.fixture, тест получает результат просто указав имя фикстуры как аргумент — pytest сам подставит.
...
def user():
    u = User(name="Alice")
    yield u          # всё до yield — setup, после — teardown
    u.delete()        # выполнится после теста, даже если тест упал

def test_user_name(user):
    assert user.name == "Alice"
```

`parametrize` — запуск одного теста с разными наборами входных данных без копипасты:
...
`conftest.py` — файл, куда выносят фикстуры и хуки, общие для нескольких тестовых файлов в директории. pytest подхватывает его автоматически, импортировать не нужно — фикстуры из conftest.py доступны во всех тестах в этой папке и подпапках.

`monkeypatch` — встроенная фикстура для временной подмены атрибутов/переменных окружения/функций на время теста, с автоматическим откатом после:
...
- внутренне само идентифицируем (независим от именования).
...
Функция `map` применяет функцию к каждому элементу последовательности. В Python возвращает объект-итератор.

Функция `filter` оставляет лишь те элементы последовательности, для которых заданная функция истинна.

Функция `reduce` (находится в модуле functools) принимает функцию от двух аргументов, последовательность и опциональное начальное значение и вычисляет свёртку (fold) последовательности как результат последовательного применения данной функции к текущему значению (так называемому аккумулятору) и следующему элементу последовательности.
...
AsyncIO — это способ писать конкурентный код без использования потоков. Вместо потоков используются корутины (сопрограммы) — лёгкие «потоки на уровне пользователя», которые переключаются между собой сами, без участия операционной системы.
Главное отличие от потоков: корутина выполняет только полезную работу. Переключение на другую корутину происходит только тогда, когда текущая чего-то ждёт — ответа от базы данных, сети, файловой системы. В это время управление передаётся следующей корутине через await.
Потоки переключаются принудительно — планировщик ОС прерывает их по таймеру, даже если поток заблокирован GIL и не делает ничего полезного. Это дорого: каждое переключение контекста требует сброса регистров, кэша процессора, таблицы памяти. При сотнях потоков процессор тратит значительную часть времени впустую.
Когда использовать asyncio:

- когда приложение большую часть времени ждёт — вебсокеты, long polling, медленные внешние API, запросы к базе данных. То есть задачи I/O-bound.
- 
Когда asyncio не поможет:
- для CPU-bound задач (вычислений) он даже хуже потоков. Поток хотя бы отпустит GIL по таймеру, а блокирующий код внутри корутины заморозит весь event loop до завершения. Поэтому в asyncio-приложениях нельзя использовать блокирующие вызовы — только асинхронные библиотеки, а тяжёлые вычисления выносить в отдельные процессы.
...
### Модуль multiprocessing: Process, Pool, Queue

`multiprocessing` создаёт настоящие отдельные процессы (в отличие от threading), поэтому обходит GIL и даёт реальный параллелизм на несколько ядер CPU — подходит для CPU-bound задач.
Process — запустить одну функцию в отдельном процессе:
...
Pool — пул воркеров для параллельной обработки коллекции данных, самый частый способ использования на практике:
...
with Pool(processes=4) as pool:
    ...
    # results: [1, 4, 9, 16, 25], посчитано параллельно на 4 процессах
```

Queue — процессы не разделяют память, поэтому обычные переменные между ними не видны. Queue — потокобезопасный/процессобезопасный способ передавать данные между процессами:

```python
from multiprocessing import Process, Queue

def worker(q):
    q.put("результат из дочернего процесса")
...
Важный нюанс: каждый процесс — это копия памяти родителя на момент старта (на Linux, через fork), поэтому передавать между процессами можно только то, что умеет сериализоваться через pickle; несериализуемые объекты (например, открытый сокет) через Queue/args не передать.
...
### Django. Что такое Middleware, для чего, как реализуется
...
Если коротко, то метаклассы - это классы, которые конструируют другие классы. Объявляются они через атрибут класса `__metaclass__` (в джанго через слой совместимости с python 3 через модуль six до версии 2).
...
Вообще вся эта магия с метаклассами очень важна в джанго и поэтому лучше саму логику становления класса не переопределять.

Если взять за пример Serializer, то можно посмотреть на код джанго:
...
**Связанность модулей (coupling)**, часто называемую "зацеплением", характеризует степень независимости модулей. 
...
Это требование вытекает из одного из основных принципов системного подхода, требующего минимизации информационных потоков между подсистемами.
...
В идеале модуль должен выполнять одну единственную функцию и иметь минимальное число «ручек управления». 
Примером модуля имеющего максимальную связность является модуль проверки орфографии, вычисления заработной платы сотрудника, вычисления логарифма функции. 
Если связанность является характеристикой системы, то связность характеризует отдельно взятый модуль.
...
Этот паттерн часто применяется в программах рисования, например Inkscape, для поддержки группировки и разгруппирования. Его полезность в таких ситуациях объясняется тем, что подлежащие группировке или разгруппировке компоненты могут быть как одиночными (например прямоугольник) так и составными (например, рожица, составленная из нескольких разных фигур)
...
В стандартной библиотеке Python имеется несколько модулей для работы со сжатыми файлами - gzip, tar+gzip, zip. Все они имеют разные интерфейсы. Если мы хотим иметь один унифицированный интерфейс для работы с архивами, мы можем воспользоваться паттерном Фасад.
...
4. интеллектуальная ссылка, которая выполняет "дополнительные действия при обращении к объекту" (можно также реализовать с помощью дескриптопа @property)
...
Паттерн Наблюдатель поддерживает отношения зависимости многие-ко-многим между объектами, например, когда изменяется состояние одного объекта, уведомляются все связанные с ним объекты. В наши дни, наверное самый распространенный пример этого паттерна и его вариантов - парадигма модель-представление-контроллер (MVC). В этой парадигме модель описывает данные, одно или несколько представлений эти данные визуализируют, а одни или несколько контроллеров осуществляют посредничество между вводом (например, взаимодействие с пользователем) и моделью. И любые изменения в модели автоматически отражаются в ассоциированных с ней представлениях.
...
Паттерн Состояние предназначен для создания объектов, поведение которых изменяется при изменении состояния; это означает, что у объекта есть режим работы. Извне создается впечатление, что изменился класс объекта.
...
Паттерн Посетитель используется, когда нужно применить некую функцию к каждому элементу коллекции или объекту-агрегатору. Это не то же самое, что типичное использование паттерна Итератор - где мы обходим коллекцию или агрегат и вызываем некий метод для каждого элемента, поскольку в случае "посетителя" вызывается внешняя функция, а не метод.

В Python имеется встроенная поддержка этого паттерна. Например, конструкция `new_list = map(function, old_sequence)` означает, что `function()` вызывается для каждого элемента `old_sequence`, в результате чего порождается `new_list`

```python
"""
Посетитель (Visitor) - паттерн поведения объектов.
...
`@lru_cache` — декоратор из модуля `functools`, который кэширует результаты вызовов функции. Если функция уже вызывалась с теми же аргументами, Python возвращает сохранённый результат без повторных вычислений.
Работает как словарь `аргументы → результат`. Когда кэш заполнен, вытесняет наименее недавно использованную запись (Least Recently Used).

Параметры:
- `maxsize=128` — максимум записей в кэше. `None` = без ограничений (кэш растёт бесконечно)
- `typed=False` — если `True`, то `f(3)` и `f(3.0)` кэшируются отдельно
 
```python
from functools import lru_cache
 
# Без кэша: 2^40 вызовов для fib(40)
...
fib.cache_clear()       # очистить кэш вручную
```
Методы кэша
 
```python
func.cache_info()   # → hits, misses, maxsize, currsize
func.cache_clear()  # → сбросить весь кэш
func.__wrapped__    # → оригинальная функция без кэша
```
Важные ограничения
 
- **Аргументы должны быть хэшируемыми** — нельзя передавать `list`, `dict`. Только `tuple`, `str`, `int` и т.д.
- **Потокобезопасен** — можно использовать в многопоточном коде, блокировки встроены
- **Живёт в памяти** — кэш сбрасывается при перезапуске процесса, не для персистентного хранения
`lru_cache` идеален для рекурсии (Фибоначчи, динамическое программирование) и чистых функций без побочных эффектов.  
Для методов класса использовать `functools.cached_property` или `methodtools.lru_cache`, так как `self` должен быть хэшируемым.
...
gRPC — это высокопроизводительный фреймворк разработанный компанией Google для вызова удаленных процедур (RPC), работает поверх HTTP/2.
...
- HTTP/2 в качестве транспорта. И это невероятно мощный ход! Вся прелесть полного сжатия данных, контроля трафика, инициации событий с сервера, переиспользования одного сокета для нескольких параллельных запросов — красотища.
...
- Поддержка девяти языков: C, C++, Java, Go, Node.js, Python, Ruby, Objective-C, PHP, C# плюс, конечно, никто не запрещает взять и реализовать свою версию.
...
О-большое (Big O) описывает, как растёт число операций алгоритма при росте размера входных данных — то есть не точное время работы, а порядок роста в худшем случае. 
Позволяет сравнивать алгоритмы независимо от железа, на котором они запущены.

Основные классы сложности от быстрого к медленному:

O(1) — константа, не зависит от размера входа (обращение к элементу списка по индексу)
O(log n) — логарифмическая (бинарный поиск)
O(n) — линейная (простой перебор списка)
O(n log n) — линейно-логарифмическая (эффективная сортировка, например sorted())
O(n²) — квадратичная (вложенный перебор, например сортировка пузырьком)
...
O(log n): работает только с отсортированным массивом. Берём средний элемент и сравниваем с целевым: совпал — нашли. 
Меньше целевого — отбрасываем левую половину, больше — правую. Повторяем до нахождения.
...
Используется для нахождения пути с наименьшим весом во взвешенном графе.
...
Используется в машинном обучении, построении рекомендательных систем, прогнозировании и так далее.
...
Непрерывная интеграция заключается в следующем: все изменения, вносимые в код, объединяются в центральном репозитории (операция называется «слияние»). Слияние происходит несколько раз в день, и после каждого слияния в конкретном проекте срабатывает автоматическая сборка и тестирование.
...
Апеллировать к прибыльности для бизнеса заказчика.
...
**Historical Branches**
Вместо использования только одной master ветки, этот workflow использует две ветки, для записи истории проекта. Master хранит официальную историю релизов, в development же ветке ведется активная разработка. Также важно тегировать комиты в master ветки номерами версий.
...
Создание этого бранча начинает новый релиз-цикл, поэтому в эту ветку уже не должно попадать никаких новых фич - только баг-фиксы, генерация документации и другие таски, ориентированные на релиз.

Как только релиз-ветка подготовлена, ее сливают с master и тегают новой версией. Также она должна быть слита и с development веткой, которая уже скорее всего убежала вперед, с момента инициализации релиза.

Использование отдельной ветки для релиза позволяет одной команде готовить релиз, в то время когда другая команда будет продолжать разрабатывать новые фичи в development для последующих релизов.
...
После манипуляций с rebase потребуется push с опцией -F. Все это потому, что мы переписываем меняем историю комитов и git об этом честно предупреждает.
`$ git push -f`
...
Такой вариант лучше тем, что если кто-то успел запушить свои коммиты после того, как мы забирали изменения с сервера, то он не будет их перетирать, а выдаст ошибку, после чего мы сможем интегрировать чужие коммиты со своими изменениями и попытаться сделать `push --force-with-lease` ещё раз.
...
14. Оцениваются ли задачи по времени или стори-поинтам?
